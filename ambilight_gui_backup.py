...
class EdgeRGBGUI(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        top.pack(fill="x", padx=22, pady=(18, 10))
                ...
                # ---- Title image (above Monitor dropdown) ----
        try:
            img = Image.open(asset_path("title.png")).convert("RGBA")
            # optional: resize to fit nicely (adjust width as you like)
            target_w = 520
            w, h = img.size
            if w > target_w:
                img = img.resize((target_w, int(h * (target_w / w))), Image.LANCZOS)

            self.title_ctk_img = ctk.CTkImage(light_image=img, dark_image=img, size=img.size)
            self.title_label = ctk.CTkLabel(top, text="", image=self.title_ctk_img)
            self.title_label.pack(pady=(0, 10))
        except Exception as e:
            print("Could not load title.png:", e)
